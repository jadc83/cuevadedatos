        ...
        <div class="bg-black bg-opacity-70 p-10 flex items-center justify-center">
            <div class="w-full max-w-4xl bg-gray-850 rounded-lg shadow-lg overflow-hidden">
                <header class="bg-gray-700 bg-opacity-45 p-8 text-center border-b-4 border-indigo-600">
                    <h1 class="text-6xl font-extrabold text-white tracking-wider uppercase">{{ ucfirst($libro->titulo) }}</h1>
                </header>

                <main class="p-8 space-y-10">
                    <!-- Sección de detalles -->
                    <section class="bg-gray-800 p-6 bg-opacity-45 rounded-lg shadow-md border-l-4 border-indigo-500">
                        <h2 class="text-4xl font-semibold text-indigo-400 text-center mb-6">Detalles del Tomo</h2>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-10 text-gray-300">
                            <div><strong>Título:</strong> {{ ucfirst($libro->titulo) }}</div>
                            ...
                            <ul class="divide-y divide-gray-600">
                                @foreach ($libro->hechizos as $hechizo)
                                    <li class="py-4 hover:bg-gray-700 transition-colors duration-200">
                                        <a href="{{ route('hechizos.show', $hechizo) }}" class="block text-blue-400 hover:text-blue-300 text-center font-semibold text-lg">
                    ...
                    <div class="flex justify-between mt-8">
                        <a href="{{ route('libros.index') }}" class="px-6 py-3 bg-indigo-600 text-white rounded-lg shadow-md hover:bg-indigo-500 transition duration-200 transform hover:scale-[1.03]">
                            Regresar al Grimorio
                        </a>
                        <a href="{{ route('libros.edit', $libro) }}" class="px-6 py-3 bg-black text-white rounded-lg shadow-md hover:bg-gray-800 transition duration-200 transform hover:scale-[1.03]">
                            Alterar
                        </a>
