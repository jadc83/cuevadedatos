    ...
    <div class="min-h-screen h-full bg-gray-900 p-2" style="background-image: url('/images/fondo_objetos.jpeg'); background-size: cover; background-position: center;">
        <div class="w-full max-w-5xl mx-auto p-2 bg-transparent rounded-lg">
                        ...
                        @foreach ($tipoArmas as $tipoArma)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-black bg-white">
                                    <a href="{{ route('tipoArmas.edit', $tipoArma) }}" class="px-4 py-3 text-black bg-white">Editar</a>
                                    <form method="POST" action="{{ route('tipoArmas.destroy', $tipoArma) }}" class="inline">
                                        @csrf
                                        @method('DELETE')
            ...
            <div class="mt-8 text-white">
                {{ $tipoArmas->links('pagination::tailwind') }}
            </div>
        </div>
    </div>
</x-app-layout>
